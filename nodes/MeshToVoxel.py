...
class MeshToVoxel:
    ...
    def run(self, mesh, voxel_size): 
        ...
        # Calculate the scale to fit the mesh inside the NxNxN voxel grid
...
class VoxelViewer:
    ...
    def run(self, voxel_block: np.ndarray): 
        ...
        return {"ui": {"voxel_block": flattened_block}}
...
class VoxelBlockSaver:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "voxel_block": ("VOXEL_BLOCK",),
                "filename_prefix": ("STRING", {"default": "voxel_blocks"}),
            },
        }
    ...
    def run(self, voxel_block: np.ndarray, filename_prefix: str):
        """
        ...
        output_dir = f"output/{filename_prefix}"
